...
var _html2json = __webpack_require__(15);

var _html2json2 = _interopRequireDefault(_html2json);

var _showdown = __webpack_require__(18);
...
Component({
    ...
    nodes: {
      ...
      observer: function observer(val) {
        ...
        if (language === 'markdown' || language === 'md') {
          ...
          setTimeout(function () {
            _this._parseNodes(parseNodes);
          }, 0);
        } else {
          // 默认采用html解析
          setTimeout(function () {
            _this._parseNodes(val);
          }, 0);
        }
      }
    }
  ...
  lifetimes: {
    detached: function detached() {
      ...
      _util.cacheInstance.removeAllByKey(this.data.pageKey);
    }
  },
  ...
  methods: {
    _parseNodes: function _parseNodes(nodes) {
      if (typeof nodes === 'string') {
        ...
        var nodesData = Object.prototype.toString.call(nodes) === '[object Array]' ? nodes : [nodes];
        this.setData({
          nodesData: nodesData
        });
      }
    ...
    _parseHtml: function _parseHtml(html) {
      ...
      var allPages = getCurrentPages(),
          ...
          wxparseRootKey = pageNodeKey + '_' + (0, _util.getUniqueKey)();

      //存放html节点转化后的json数据
      var transData = _html2json2.default.html2json(html, wxparseRootKey);
      transData.view = {};
      transData.view.imagePadding = 0;

      this.setData({
        wxparseRootKey: wxparseRootKey,
        pageNodeKey: pageNodeKey,
        nodesData: transData.nodes,
        bindData: (_bindData = {}, _bindData[wxparseRootKey] = transData, _bindData)
      });
      // 构建page页面对象内部的wxparse富文本节点数组集合 eg: pageId => [wxparse1key, wxparse2key, ....]
      var pageInstance = _util.cacheInstance.get(pageNodeKey);
      if (pageInstance) {
        pageInstance.push(wxparseRootKey);
      } else {
        _util.cacheInstance.set(pageNodeKey, [wxparseRootKey]);
      }
      _util.cacheInstance.set(wxparseRootKey, transData);
    ...
    wxParseImgTap: function wxParseImgTap(e) {
      var _ref2 = e.target.dataset || {},
          _ref2$src = _ref2.src,
          src = _ref2$src === undefined ? '' : _ref2$src;

      var _ref3 = _util.cacheInstance.get(this.data.rootKey) || {},
          _ref3$imageUrls = _ref3.imageUrls,
          imageUrls = _ref3$imageUrls === undefined ? [] : _ref3$imageUrls;
...
var __emojisBaseSrc = '';
var __emojis = {};
var wxDiscode = __webpack_require__(16);
var HTMLParser = __webpack_require__(17);
...
function html2json(html, bindName) {
    ...
    var index = 0;
    HTMLParser(html, {
        start: function start(tag, attrs, unary, content) {
	...
	while (html) {
		...
		if (!stack.last() || !special[stack.last()]) {
			...
			html = html.replace(new RegExp("([\\s\\S]*?)<\/" + stack.last() + "[^>]*>"), function (all, text) {
				...
				if (handler.chars && text.trim() !== '') handler.chars(text);
...
var bindInstance = function bindInstance() {
  ...
  return {
     ...
     * 清楚实例对象特定的键
     */
    remove: function remove(bindName) {
     ...
     * @param {*} key 
     */
    removeAllByKey: function removeAllByKey(key) {
      if (!instance[key]) return;
      // 清除子节点关联
      for (var _iterator = instance[key], _isArray = Array.isArray(_iterator), _i = 0, _iterator = _isArray ? _iterator : _iterator[Symbol.iterator]();;) {
        var _ref;
